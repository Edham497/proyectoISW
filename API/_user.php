...
//ACTUALIZAR USUARIO
function updateUser($id,$nom,$apP,$apM,$fec,$email,$pass,$dir,$tel){
    $db = new Database();
    $conn = $db->getConn();
    $passR = password_hash($pass,PASSWORD_DEFAULT);
    $query = "UPDATE Usuario SET nomUsuario = '$nom', apPUsuario = '$apP', apMUsuario = '$apM', fecNUsuario = '$fec', email = '$email', pass = '$passR',direccion = '$dir', telefono = '$tel' WHERE idUsuario = $id;";
    $stmt = $conn->prepare($query);
    $stmt->execute();
